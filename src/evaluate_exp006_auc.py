#!/usr/bin/env python3
"""
Calculate AUC scores for exp006 experiments and ensemble
"""
...
def main():
    print("=== exp006 AUC Score Evaluation ===")
    print("")
    ...
    for seed in seeds:
        for model in models:
            exp_dir = working_dir / f"exp006_seed{seed}_{model}_{model}"
            if exp_dir.exists():
    ...
    print(f"\nEnsemble AUC score:")
    ensemble_dir = working_dir / "exp006_ensemble_weighted"
        ...
        try:
            ...
            if "target_y" in merged.columns and "target_x" in merged.columns:
                ...
                if len(y_true) > 0:
                    ...
                    print(f"  Ensemble (LGB:0.6, CAT:0.3, XGB:0.1): {ensemble_auc:.6f}")
                else:
